...
import { normalizeAlertmanager, normalizeGrafana, normalizeInflux, normalizeServarr, normalizeSeerr, normalizeUptimeKuma, mapAlertmanagerSeverity, mapGrafanaSeverity, mapInfluxSeverity, mapServarrSeverity, } from "./webhooks.js";
/* ------------------------------------------------------------------ */
/*  Fixtures                                                           */
/* ------------------------------------------------------------------ */
...
describe("normalizeInflux", () => {
    ...
    it("should map ok level → resolved status", () => {
        const okPayload = { ...influxNotification, _level: "ok" };
...
describe("normalizeUptimeKuma", () => {
    ...
    it("should skip payloads with missing heartbeat or monitor", () => {
        ...
        expect(normalizeUptimeKuma({ heartbeat: { status: 0, msg: "down", time: "now" } })).toHaveLength(0);
        expect(normalizeUptimeKuma({ monitor: { id: 1, name: "X" } })).toHaveLength(0);
    });
