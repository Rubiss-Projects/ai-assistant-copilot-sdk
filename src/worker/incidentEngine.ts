...
import {
  ...
  findBySourceId,
  updateStatus,
} from "../app/store/incidents.js";
import type { Incident } from "../app/store/incidents.js";
...
function severityColor(severity: string | undefined): number {
  if (severity === "critical") return 0xff0000;
  if (severity === "warning") return 0xffa500;
  return 0x3498db;
}

function buildAlertPayload(alert: NormalizedAlert) {
  return {
    content: null,
    embeds: [
      {
...
function buildResolvedPayload(incident: Incident) {
  return {
    ...
    if (existing) {
      ...
      if (isOpen) {
        if (alert.metadata) {
          ...
          const db = getDb();
          db.prepare("UPDATE incidents SET metadata = ?, updated_at = ? WHERE id = ?").run(
...
export function addTimelineEvent(
  incidentId: string,
  event: { event_type: string; actor?: string; content?: string },
): void {
  const db = getDb();
  db.prepare(
    "INSERT INTO incident_events (incident_id, event_type, actor, content) VALUES (?, ?, ?, ?)",
  ).run(incidentId, event.event_type, event.actor ?? null, event.content ?? null);
}

export function getTimeline(incidentId: string): IncidentEvent[] {
  const db = getDb();
  return db
    .prepare("SELECT * FROM incident_events WHERE incident_id = ? ORDER BY created_at ASC")
    .all(incidentId) as IncidentEvent[];
}
...
export function transitionStatus(
  ...
  if (!allowed || !allowed.includes(newStatus)) {
    throw new Error(
      `Invalid transition: ${row.status} → ${newStatus} (allowed: ${allowed?.join(", ") ?? "none"})`,
    );
  }
  ...
  if (newStatus === "resolved" || newStatus === "closed") {
    const incident = db
      ...
      .get(incidentId) as { thread_id: string | null; title: string } | undefined;
