...
export class CommandProcessor {
  ...
  constructor(
    options: { pollIntervalMs?: number; escalationChannelId?: string } = {},
  ) {
  ...
  start(): void {
    ...
    console.log(
      `[delivery] Command processor started (poll every ${this.pollIntervalMs}ms)`,
    );
  }
  ...
  private async execute(cmd: operatorCommands.OperatorCommand): Promise<void> {
    if (!cmd.incident_id) throw new Error("Command has no incident_id");
    ...
    switch (cmd.command_type) {
      ...
      case "note":
        addTimelineEvent(cmd.incident_id, {
          event_type: "note",
          actor: cmd.actor,
          content: cmd.payload?.text as string,
        });
        break;
      ...
      case "escalate": {
        ...
        addTimelineEvent(cmd.incident_id, {
          ...
          actor: cmd.actor,
          content: cmd.payload?.reason as string,
        });
        break;
      }
