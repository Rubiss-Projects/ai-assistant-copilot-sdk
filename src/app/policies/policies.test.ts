...
describe("maintenance window", () => {
  const config: MaintenanceWindowConfig = {
  ...
  it("should handle midnight wrapping", () => {
    const wrapConfig: MaintenanceWindowConfig = {
      dayOfWeek: [5], // Friday
      startHour: 22,
      endHour: 4,
    };
...
describe("policy engine evaluate", () => {
  ...
  it("should evaluate rules in order, first denial wins", () => {
    ...
    engine.addRule({
      type: "denylist",
      name: "deny-delete",
      config: { actions: ["delete"], services: ["*"] } satisfies AllowDenyConfig,
    });
    engine.addRule({
      type: "denylist",
      name: "deny-destroy",
      config: { actions: ["destroy"], services: ["*"] } satisfies AllowDenyConfig,
    });
  ...
  it("should return requiresApproval for maintenance window", () => {
    ...
    engine.addRule({
      ...
      } satisfies MaintenanceWindowConfig,
    });
...
describe("cleanup", () => {
  ...
  it("should purge expired idempotency keys", () => {
    ...
    ).run("old-key", new Date(Date.now() - 100_000).toISOString());
    db.prepare(
      "INSERT INTO idempotency_keys (key, expires_at) VALUES (?, ?)",
    ).run("fresh-key", new Date(Date.now() + 100_000).toISOString());
  ...
  it("should purge old resolved incidents", () => {
    ...
    db.prepare("UPDATE incidents SET resolved_at = ? WHERE id = ?").run(
  ...
  it("should purge old audit events", () => {
    const db = getDb();
    db.prepare(
      "INSERT INTO audit_events (process, event_type, created_at) VALUES (?, ?, ?)",
    ).run("test", "old-event", new Date(Date.now() - 100 * 24 * 60 * 60 * 1000).toISOString());
    db.prepare(
      "INSERT INTO audit_events (process, event_type, created_at) VALUES (?, ?, ?)",
    ).run("test", "fresh-event", new Date().toISOString());
