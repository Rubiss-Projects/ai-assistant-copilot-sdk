...
export interface CleanupConfig {
  idempotencyKeyTtlMs: number; // Default: 24 hours
  resolvedIncidentRetentionMs: number; // Default: 30 days
...
export function runCleanup(
...
): { idempotencyKeys: number; incidents: number; auditEvents: number } {
  ...
  const cutoff = new Date(
    Date.now() - cfg.resolvedIncidentRetentionMs,
  ).toISOString();
  const oldIncidentIds = db
    .prepare(
      "SELECT id FROM incidents WHERE status IN ('resolved', 'closed') AND resolved_at < ?",
    )
  ...
  if (oldIncidentIds.length > 0) {
    ...
    db.prepare(
      `DELETE FROM operator_commands WHERE incident_id IN (${placeholders})`,
    ).run(...ids);
    db.prepare(
      `DELETE FROM approval_decisions WHERE incident_id IN (${placeholders})`,
    ).run(...ids);
    db.prepare(
      `DELETE FROM notifications_outbox WHERE thread_id IN (SELECT thread_id FROM incidents WHERE id IN (${placeholders}))`,
    ).run(...ids);
    incidents = db
  ...
  const policyStateCutoff = new Date(
    Date.now() - cfg.idempotencyKeyTtlMs,
  ).toISOString();
  db.prepare(
    "DELETE FROM plugin_state WHERE plugin_name IN ('_policy_engine', '_dedupe') AND updated_at < ?",
  ).run(policyStateCutoff);
